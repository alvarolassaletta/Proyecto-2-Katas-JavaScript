/**Crea una función que reciba un array de objetos representando a varios miembros de los X-Men y su año de aparición. La función debe retornar el miembro más antiguo.
 * Utiliza este array para probar tu función.
 */

const xMen = [
  { name: 'Wolverine', year: 1974 },
  { name: 'Cyclops', year: 1963 },
  { name: 'Storm', year: 1975 },
  { name: 'Phoenix', year: 1963 },
  { name: 'Beast', year: 1963 },
  { name: 'Gambit', year: 1990 },
  { name: 'Nightcrawler', year: 1975 },
  { name: 'Magneto', year: 1963 },
  { name: 'Professor X', year: 1963 },
  { name: 'Mystique', year: 1978 }
];

function findOldestXMen(xMen) {

    if(!Array.isArray(xMen)){
        return 'el argumento debe ser un array';
    }

    let oldestDate=xMen[0].year, oldestMember="";

    for(let i=0;i<xMen.length;i++){
        if(xMen[i].year <= oldestDate){
            oldestDate=xMen[i].year;
            oldestMember=xMen[i].name;
        }
    }
    return oldestMember;
}

let oldestXmen= findOldestXMen(xMen);
console.log(oldestXmen);