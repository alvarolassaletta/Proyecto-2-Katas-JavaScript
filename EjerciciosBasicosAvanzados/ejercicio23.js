...
const shortFilm=[], mediumLengthFilm=[],fullLengthFilm=[];

for ( let i=0;i<movies.length;i++){
    if(movies[i].durationInMinutes <100){
        shortFilm.push(movies[i]);
    } else if (movies[i].durationInMinutes >=100 && movies[i].durationInMinutes <200){
        mediumLengthFilm.push(movies[i]);
    } else{
        fullLengthFilm.push(movies[i]);
    }
}

console.log("\nPeliculas de corta duracion ")
for(let item of shortFilm){
    console.log(item);
}

console.log("\nPeliculas de duracion media")
for(let item of mediumLengthFilm){
    console.log(item);
}

console.log("\nPeliculas de larga duracion")
for(let item of fullLengthFilm){
    console.log(item);
}
